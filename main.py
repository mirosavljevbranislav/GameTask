...
def bouncing_ball():
    ...
    ball = pygame.image.load("download.webp")
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if clicks + 1 < len(squares):
                    x, y = event.pos
                    if squares[clicks][0] <= x <= squares[clicks][0] + width // 5 \
                            and squares[clicks][1] <= y <= squares[clicks][1] + height // 5:
                        clicks += 1
                else:
                    corrects += 1
                    generate_square = True
        ...
        for j, column in enumerate(row):
            if column == '0':
                pygame.draw.rect(surface,
                                 shape.color,
                                 (int(sx + j * block_size), int(sy + i * block_size), block_size, block_size),
                                 0)
...
def draw_window(surface, grid, score=0, last_score=0):
    ...
    surface.blit(score_label, (sx + 20, sy + 50))
    ...
    draw_grid(surface=surface, grid=grid)
    # pygame.display.update()
...
def main(surface):
    ...
    while run:
        ...
        draw_window(surface=surface, grid=grid, score=score, last_score=int(last_score))
        draw_next_shape(next_piece, surface)
        pygame.display.update()

        if check_lost(locked_positions):
            draw_text_middle(surface, "GG loser EZ!", 80, (255, 255, 255))
            pygame.display.update()
